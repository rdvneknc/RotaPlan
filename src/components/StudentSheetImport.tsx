...
interface Props {
  ...
  /** Servis hesabı e-postası — dosyayı paylaşma hatırlatması */
...
export default function StudentSheetImport({
  ...
  const disabled = !googleSheetsConfigured || loading;
    ...
    <div className="rounded-xl border border-dark-500 bg-dark-700/40 p-4 space-y-3">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1 leading-relaxed">
          Haftalık program dosyasından bağımsız bir tablo kullanabilirsiniz. Okul ayarındaki linkten farklı bir
          dosya yapıştırmanız yeterli; servis hesabına bu dosyada da <strong>Düzenleyici</strong> ile paylaşın.{" "}
          <span className="text-gray-400">A</span> ad soyad, <span className="text-gray-400">B</span> Maps linki,{" "}
          <span className="text-gray-400">C–D</span> irtibat 1, <span className="text-gray-400">E–F</span> irtibat 2.
          İlk satır başlık; veri 2. satırdan.
        </p>
      </div>

      {!googleSheetsConfigured && (
        <p className="text-xs text-amber-400/90">
          Sunucuda Google servis hesabı tanımlı değil (GOOGLE_SERVICE_ACCOUNT_JSON); içe aktarma çalışmaz.
        </p>
      )}

      {googleSheetsConfigured && shareEmail && (
        <p className="text-xs text-gray-400 leading-relaxed">
          <span className="text-gray-500">Sheets dosyasını bu adresle paylaşın:</span>{" "}
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div>
          <label htmlFor="student-import-sheet-url" className="block text-xs font-medium text-gray-400 mb-1">
            Sheets linki veya dosya ID
          </label>
          <input
            id="student-import-sheet-url"
            name="googleSheetUrl"
            type="text"
            value={sheetUrl}
